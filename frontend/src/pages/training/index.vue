<template>
  <view class="training-page">
    ...
    <template v-else>
      <liquid-glass-card class="empty-plan-card" :highlight="true">
        <view class="empty-plan-icon"><line-icon name="dumbbell" color="#3FA67C" :size="72" /></view>
        <view class="empty-plan-copy">
          <view class="empty-title">还没有训练计划</view>
          <view class="empty-sub">从模板开始，按自己的训练节奏继续调整。</view>
        </view>
        <liquid-glass-button text="新建训练计划" variant="primary" @tap="goCreatePlan" />
      </liquid-glass-card>
    </template>
  </view>
</template>
...
.empty-plan-card { width: 100%; box-sizing: border-box; margin-top: 44rpx; padding: 40rpx 32rpx 32rpx; }
.empty-plan-icon { width: 128rpx; height: 128rpx; display: flex; align-items: center; justify-content: center; border-radius: 40rpx; background: $primary-tint; }
.empty-plan-copy { margin: 22rpx 0 28rpx; }
.empty-title { font-size: $fs-xl; color: $text-1; font-weight: 750; }
.empty-sub { margin-top: 8rpx; color: $text-3; font-size: $fs-sm; line-height: 1.6; }
</style>
